%define module fcdslusb
%define version 3.11.07
%define card "AVM GmbH|FtitzCard USB DSL ISDN TA/ DSL Modem"
...
Source1: dkms-fcdslusb-use-autoconf-header.patch
Source2: dkms-fcdslusb-dont-redefine-uintptr_t.patch
Source3: dkms-fcdslusb-remove-usb_driver_owner.patch
Source4: dkms-fcdslusb-remove-DECLARE_MUTEX_LOCKED.patch
Source5: dkms-fcdslusb-update-INIT_WORK-changes.patch
Source6: dkms-fcdslusb-update-usb_complete_t-functions.patch
...
CLEAN="make clean"
AUTOINSTALL="yes"
PATCH[0]="dkms-fcdslusb-use-autoconf-header.patch"
PATCH_MATCH[0]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[1]="dkms-fcdslusb-dont-redefine-uintptr_t.patch"
PATCH_MATCH[1]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[2]="dkms-fcdslusb-remove-usb_driver_owner.patch"
PATCH_MATCH[2]="^2\.6\.(1[6-9])|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[3]="dkms-fcdslusb-remove-DECLARE_MUTEX_LOCKED.patch"
PATCH_MATCH[3]="^2\.6\.(2[4-9])|([3-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[4]="dkms-fcdslusb-update-INIT_WORK-changes.patch"
PATCH_MATCH[4]="^2\.6\.([2-9][0-9]+)|([1-9][0-9][0-9]+)"
PATCH[5]="dkms-fcdslusb-update-usb_complete_t-functions.patch"
PATCH_MATCH[5]="^2\.6\.(19)|([2-9][0-9]+)|([1-9][0-9][0-9]+)"
EOF

tar c . | tar x -C $RPM_BUILD_ROOT/usr/src/%module-%version-%release/

for p in %{_sourcedir}/dkms-fcdslusb-use-autoconf-header.patch \
         %{_sourcedir}/dkms-fcdslusb-dont-redefine-uintptr_t.patch \
         %{_sourcedir}/dkms-fcdslusb-remove-usb_driver_owner.patch \
         %{_sourcedir}/dkms-fcdslusb-remove-DECLARE_MUTEX_LOCKED.patch \
         %{_sourcedir}/dkms-fcdslusb-update-INIT_WORK-changes.patch \
         %{_sourcedir}/dkms-fcdslusb-update-usb_complete_t-functions.patch;
do
